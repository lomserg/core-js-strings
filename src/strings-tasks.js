...
function isPalindrome(str) {
  ...
  const strRevers = [...newStr].reverse().join('');
  const orig = newStr.split(',').join('').replace(/\s/g, '').toLowerCase();
  return (
    orig === strRevers.split(',').join('').replace(/\s/g, '').toLowerCase()
  );
}
/**
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    const rotate13 = (index + 13) % alphabet.length;
    const letter = str[i];
    if (index !== -1) {
      if (letter.toUpperCase() === letter) {
        result += [...alphabet].at(rotate13).toUpperCase();
      } else {
        result += [...alphabet].at(rotate13).toLowerCase();
      }
    } else {
      result += letter;
    }
